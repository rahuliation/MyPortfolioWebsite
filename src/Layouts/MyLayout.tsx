...
const MenuItem = (props: {
  ...
  props.active ? (
    <span
      className="f7 f5-ns ttc tracked-mega link dib pv2-ns ph4-ns mb2 pv1 ph2 b menuActive primary pointer"
      key={props.in}
    >
      <div className={`b`}>
  ...
  <div className="w-100 bg-white shadow-2 navy mmenu pv2" ref={props.navRef}>
    <nav className="tc mw7 center">
      {props.menus.map((menu: any, key: number) => (
        <MenuItem {...menu} in={key} active={location.pathname === menu.path} />
      ))}
    </nav>
  </div>
);

export const Social = (props: { className?: string; hoverClass?: string }) => (
  ...
  return (
    <div className="fl w-100 bg-white2" dir="ltr">
      <div className="pv4 ph2 bg-navy2 near-white pv6-l fixed w-25-l w-100 vh-100 sidebar">
        ...
        <div
          className="tc"
          style={{ fontFamily: "Viga, sans-serif", fontWeight: 300 }}
        >
          <span className="f1-ns f3">{staticData.name}</span>
          <br />
        ...
        <div className="absolute hidden-h500 w-100 bottom-2 overflow-x-hidden tc">
          ...
          <Social
            className="f6 pt3 w-100 cf"
            hoverClass="hover-bg-white hover-navy"
          />
        </div>
      </div>
